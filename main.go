...
var (
	...
	redirectHTTP = flag.Bool("redirectHTTP", false, "if foo, redirects http requests from port 80 to https at your fromURL")
	useTailscale = flag.Bool("useTailscale", false, "if true, uses tailscale to get the certs")
)
...
func main() {
	...
	if validDomain && !*useTailscale {
		...
		// TODO: validate domain (though, autocert may do this)
		// TODO: for some reason this seems to only work on :443
		log.Printf("Domain specified, using Tailscale to get certs")
		if !strings.HasSuffix(*fromURL, ":443") {
			log.Println("WARN: Right now, you must serve on port :443 to use autogenerated LetsEncrypt certs using the -domain flag, this may NOT WORK")
		}
		s := &http.Server{
